...
if (
    ...
    st.info(
        "환경 요인 중요도 분석은 앞 단계의 모델 학습이 완료되면 실행됩니다."
    )
...
# 현재 앞 코드에서는 최종 데이터가 final_df로 만들어짐
if "final_df" in globals() and final_df is not None:

    analysis_df = final_df

    # material과 target이 모두 있어야 분석 가능
    if (
        "material" in analysis_df.columns
        and "target" in analysis_df.columns
    ):

        # 실제 데이터에 존재하는 환경 변수만 사용
        available_features = [
            feature
        ...
        if len(available_features) == 0:
            ...
            for material in materials:
                ...
                # target 값이 하나뿐이면 학습 불가
                if y_sub.nunique() < 2:

                    st.warning(
                        f"{material} 문화재의 target 값이 "
                        "한 종류뿐이라 분석할 수 없습니다."
                    )
