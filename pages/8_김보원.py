...
# ==========================================================
# 2. 데이터 불러오기 및 전처리 (경로 정확히 수정)
# ==========================================================
@st.cache_data
def load_data():
    # pages/ 폴더 내부 파일 경로 지정
    possible_paths = [
    ...
    if df is None:
        raise FileNotFoundError("CSV 파일을 찾을 수 없습니다. 경로를 확인해주세요.")

    # 좌표 숫자형 변환
    df["위도"] = pd.to_numeric(df["위도"], errors="coerce")
    df["경도"] = pd.to_numeric(df["경도"], errors="coerce")
    
    # 결측치 제거 및 좌표가 0인 데이터 제거 (영천 지역 좌표만 남김)
...
if show_cluster:
    ...
    for _, row in yc_map_df.iterrows():
        ...
        <div style="width:230px; font-family: sans-serif;">
            <h4 style="margin-bottom:8px; color:#1f77b4;">{row.get('문화재명(국문)', '문화재')}</h4>
            <b>• 국가유산종목:</b> {row.get('국가유산종목', '-')}<br>
            <b>• 시대:</b> {row.get('시대', '-')}<br>
            <b>• 소재지:</b> {row.get('소재지상세', '-')}<br>
            <b>• 관리자:</b> {row.get('관리자', '-')}
        </div>
        """
